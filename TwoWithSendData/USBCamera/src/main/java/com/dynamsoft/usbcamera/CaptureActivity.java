...
public class CaptureActivity extends BaseActivity implements CameraDialog.CameraDialogParent {
    ...
    private UVCCamera firstCamera;

    private UVCCamera secondCamera;
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        firstCameraButton.setOnClickListener(firstButton);

        secondCameraButton = (Button) findViewById(R.id.second_button);
        secondCameraButton.setOnClickListener(secondB);
    ...
    private final View.OnClickListener firstButton = new View.OnClickListener() {
        @Override
        public void onClick(final View view) {
...
//                if (mUSBMonitor.hasPermission(devices.get(0)))
//                    mUSBMonitor.requestPermission(devices.get(0));
//                mUSBMonitor.openDevice(devices.get(0));
            }
        }
    };

    private final View.OnClickListener secondB = new View.OnClickListener() {
        @Override
        public void onClick(final View view) {
            Log.d(TAG, "secondB");
            Log.d(TAG, String.valueOf(devices.size()));
            if(devices.size() == 2) {
                if(secondBlock != null){
                    ...
                    USBMonitor.UsbControlBlock ctrlBlock = mUSBMonitor.openDevice(devices.get(1));

                    startPreview(ctrlBlock);
                }
//                if (mUSBMonitor.hasPermission(devices.get(1)))
//                    mUSBMonitor.requestPermission(devices.get(1));
//                mUSBMonitor.openDevice(devices.get(1));
            }
        }
    };
    ...
    private final USBMonitor.OnDeviceConnectListener mOnDeviceConnectListener = new USBMonitor.OnDeviceConnectListener() {
        ...
        public void onConnect(final UsbDevice device, final USBMonitor.UsbControlBlock ctrlBlock, final boolean createNew) {
            ...
            synchronized (mSync) {
                if(firstBlock == null && device.getDeviceName().equals(devices.get(0).getDeviceName()))
                    firstBlock = ctrlBlock;

                if(secondBlock == null && device.getDeviceName().equals(devices.get(1).getDeviceName()))
...
//                mCameraHandler.captureStill("qwer" + i);
            }
        }
    }
